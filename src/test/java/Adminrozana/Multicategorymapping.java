...
public class Multicategorymapping {
	@Test
	public void multicategorymapping() throws InterruptedException {
		 Logger logger = Logger.getLogger(Multicategorymapping.class.getName());
		// set the path of
		...
		WebDriver driver = new ChromeDriver();
            ...
            driver.findElement(By.xpath("//a[text()='Multi Category Mapping']")).click();
            driver.findElement(By.xpath("//input[@onkeyup='searchProduct(this.value)']")).sendKeys("FC Arhar Dal");
            WebElement Cat=driver.findElement(By.xpath("//select[@id='categories']"));
            Select select=new Select(Cat);
            select.selectByVisibleText("Egg, Meat & Fish");
            WebElement Subcat =driver.findElement(By.xpath("//select[@id='sub_categories']"));
            Select select1=new Select(Subcat);
            select1.selectByVisibleText("Poultry");
            WebElement subsubcat =driver.findElement(By.xpath("//select[@id='sub_sub_categories']"));
            Select select2=new Select(subsubcat);
            select2.selectByVisibleText("Test sub subcategory1");
